...
module.exports = {
    ...
    async execute(message, args, client, Stats) {
        ...
        memberStats = await Stats.findOne({where: {userId: message.member.user.id}});
        if(!memberStats) await Stats.create({userId: message.member.user.id}).then(s => memberStats = s);
